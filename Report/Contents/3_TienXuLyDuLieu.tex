...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Bóc tách và định hình lại cấu trúc (Data Reshaping)}: Cột \texttt{library} của mỗi người dùng ban đầu được lưu dưới dạng một mảng JSON nguyên khối (VD: \texttt{[\{"appid":730,"playtime\_mins":500\}]}). Dữ liệu này được đẩy vào bảng trung gian \texttt{stg\_library\_temp}, sau đó sử dụng Common Table Expression (CTE) kết hợp các hàm xử lý JSON của PostgreSQL để giải nén (unnest) và trải phẳng thành nhiều bản ghi riêng biệt \texttt{(playerid, appid, playtime\_mins)} trong bảng \texttt{fact\_library}.
    \item \textbf{Xử lý giá trị rỗng (Null Handling)}: Sử dụng bộ lọc trong CTE để loại bỏ hoàn toàn các bản ghi có \texttt{playtime\_mins} bị khuyết thiếu hoặc mang giá trị 'NaN' trước khi nạp vào Fact, nhằm ngăn chặn việc hệ thống gán nhầm giá trị 0 gây sai lệch trong phát hiện SAM bot.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Xử lý Null (Null handling)}: Lọc và loại bỏ hoàn toàn các bản ghi bị thiếu thông tin giờ chơi (\texttt{playtime\_mins} rỗng hoặc 'NaN') ngay từ bước CTE trước khi nạp.
