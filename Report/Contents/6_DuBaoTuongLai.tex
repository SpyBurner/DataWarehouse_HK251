...
\begin{enumerate}
    ...
    \item Xử lý feature matrix $X\_{raw}$:
    \begin{itemize}
        \item IF path: Log-transform → Preprocess (SimpleImputer + StandardScaler)
        \item XGBoost path: Sử dụng $X\_{raw}$ trực tiếp (không transform)
    \end{itemize}
    \item Prediction:
    \begin{itemize}
...
\begin{enumerate}
    \item Sampling: Chọn ngẫu nhiên 5000 rows từ $X\_{raw}$ (unscaled — giữ axis values trên thang đo gốc)
